...
const PALETTE_REGULAR = {
  ...
  shirtDark: parseHex('#1A1A1E'), // Deep black shadow
...
function drawSmallIdle(f: PixelImage, p: BodyPalette): void {
  // Torso (centered)
  box(f, 4, 2, 8, 8, p.shirt, p.outline);
  fillRect(f, 5, 3, 6, 2, p.shirtDark); // shadow fold
  // Belt
  fillRect(f, 4, 10, 8, 1, p.belt);
  // Arms at sides
  fillRect(f, 2, 3, 2, 6, p.skin);
  setPixel(f, 2, 3, p.outline);
  setPixel(f, 3, 3, p.outline);
  setPixel(f, 2, 8, p.outline);
  fillRect(f, 12, 3, 2, 6, p.skin);
  setPixel(f, 12, 3, p.outline);
  setPixel(f, 13, 3, p.outline);
  setPixel(f, 13, 8, p.outline);
  // Pants
  box(f, 4, 11, 8, 5, p.pants, p.outline);
  fillRect(f, 8, 12, 3, 3, p.pantsDark);
  // Legs split
  fillRect(f, 7, 14, 2, 2, p.outline); // gap between legs
  // Left leg
  fillRect(f, 4, 16, 4, 4, p.pants);
  setPixel(f, 4, 16, p.outline);
  setPixel(f, 4, 19, p.outline);
  // Right leg
  fillRect(f, 8, 16, 4, 4, p.pants);
  setPixel(f, 11, 16, p.outline);
  setPixel(f, 11, 19, p.outline);
  // Shoes
  fillRect(f, 3, 20, 5, 3, p.shoe);
  fillRect(f, 8, 20, 5, 3, p.shoe);
  // Shoe outline
  setPixel(f, 3, 20, p.outline);
  setPixel(f, 7, 22, p.outline);
  setPixel(f, 8, 22, p.outline);
  setPixel(f, 12, 20, p.outline);
}

function drawSmallWalk1(f: PixelImage, p: BodyPalette): void {
  // Torso
  box(f, 4, 2, 8, 8, p.shirt, p.outline);
  fillRect(f, 5, 3, 6, 2, p.shirtDark);
  fillRect(f, 4, 10, 8, 1, p.belt);
  // Arms swinging
  fillRect(f, 2, 4, 2, 5, p.skin);
  setPixel(f, 2, 4, p.outline);
  fillRect(f, 12, 2, 2, 5, p.skin);
  setPixel(f, 13, 2, p.outline);
  // Pants
  box(f, 4, 11, 8, 4, p.pants, p.outline);
  // Left leg forward
  fillRect(f, 3, 15, 4, 5, p.pants);
  setPixel(f, 3, 15, p.outline);
  fillRect(f, 2, 20, 5, 3, p.shoe);
  // Right leg back
  fillRect(f, 9, 15, 4, 4, p.pants);
  setPixel(f, 12, 15, p.outline);
  fillRect(f, 9, 19, 5, 3, p.shoe);
}

function drawSmallWalk2(f: PixelImage, p: BodyPalette): void {
  // Torso (slight bob - 1px lower start)
  box(f, 4, 3, 8, 8, p.shirt, p.outline);
  fillRect(f, 5, 4, 6, 2, p.shirtDark);
  ...
  // Pants
  box(f, 4, 12, 8, 4, p.pants, p.outline);
  // Legs together
...
function drawSmallWalk3(f: PixelImage, p: BodyPalette): void {
  ...
  // Left leg back
  fillRect(f, 3, 15, 4, 4, p.pants);
  setPixel(f, 3, 15, p.outline);
  fillRect(f, 2, 19, 5, 3, p.shoe);
}

function drawSmallRun1(f: PixelImage, p: BodyPalette): void {
  // Torso leaning forward
  box(f, 5, 2, 8, 8, p.shirt, p.outline);
  fillRect(f, 6, 3, 6, 2, p.shirtDark);
  ...
  // Pants
  box(f, 5, 11, 8, 3, p.pants, p.outline);
...
function drawSmallRun2(f: PixelImage, p: BodyPalette): void {
  ...
  box(f, 5, 3, 8, 8, p.shirt, p.outline);
  fillRect(f, 6, 4, 6, 2, p.shirtDark);
  ...
  // Pants
  box(f, 5, 12, 8, 3, p.pants, p.outline);
  // Legs together-ish
...
function drawSmallSkid(f: PixelImage, p: BodyPalette): void {
  // Torso leaning back
  box(f, 3, 2, 8, 8, p.shirt, p.outline);
  fillRect(f, 4, 3, 6, 2, p.shirtDark);
  ...
  setPixel(f, 13, 2, p.outline);
  // Pants
  box(f, 3, 11, 8, 4, p.pants, p.outline);
  // Legs wide planted
...
function drawSmallJump(f: PixelImage, p: BodyPalette): void {
  // Torso
  box(f, 4, 1, 8, 8, p.shirt, p.outline);
  fillRect(f, 5, 2, 6, 2, p.shirtDark);
  ...
  // Pants
  box(f, 4, 10, 8, 4, p.pants, p.outline);
  // Legs tucked
...
function drawSmallFall(f: PixelImage, p: BodyPalette): void {
  ...
  // Pants
  box(f, 4, 11, 8, 3, p.pants, p.outline);
  // Legs extended down
...
function drawSmallLand(f: PixelImage, p: BodyPalette): void {
  ...
  box(f, 4, 6, 8, 6, p.shirt, p.outline);
  fillRect(f, 5, 7, 6, 2, p.shirtDark);
  ...
  // Pants (short)
  box(f, 4, 13, 8, 3, p.pants, p.outline);
  // Legs bent
...
function drawSmallHurt(f: PixelImage, p: BodyPalette): void {
  // Torso recoiling
  box(f, 3, 3, 8, 8, p.shirt, p.outline);
  fillRect(f, 4, 4, 6, 2, p.shirtDark);
  ...
  setPixel(f, 13, 1, p.outline);
  // Pants
  box(f, 4, 12, 7, 4, p.pants, p.outline);
  // Legs splayed
...
function drawSmallDead(f: PixelImage, p: BodyPalette): void {
  ...
  box(f, 3, 6, 10, 6, p.shirt, p.outline);
  fillRect(f, 4, 7, 8, 2, p.shirtDark);
  ...
  setPixel(f, 14, 7, p.outline);
  // Pants
  box(f, 4, 13, 8, 4, p.pants, p.outline);
  // Legs flat
...
function drawBigIdle(f: PixelImage, p: BodyPalette): void {
  box(f, 4, 2, 8, 10, p.shirt, p.outline);
  fillRect(f, 5, 3, 6, 3, p.shirtDark);
  fillRect(f, 4, 12, 8, 1, p.belt);
  ...
  box(f, 4, 13, 8, 7, p.pants, p.outline);
  fillRect(f, 8, 14, 3, 5, p.pantsDark);
...
function drawBigWalk1(f: PixelImage, p: BodyPalette): void {
  ...
  setPixel(f, 13, 2, p.outline);
  box(f, 4, 13, 8, 5, p.pants, p.outline);
...
function drawBigWalk2(f: PixelImage, p: BodyPalette): void {
  box(f, 4, 3, 8, 10, p.shirt, p.outline);
  fillRect(f, 5, 4, 6, 3, p.shirtDark);
  fillRect(f, 4, 13, 8, 1, p.belt);
  fillRect(f, 2, 4, 2, 8, p.skin);
  setPixel(f, 2, 4, p.outline);
  fillRect(f, 12, 4, 2, 8, p.skin);
  setPixel(f, 13, 4, p.outline);
  box(f, 4, 14, 8, 5, p.pants, p.outline);
...
function drawBigRun1(f: PixelImage, p: BodyPalette): void {
  box(f, 5, 2, 8, 10, p.shirt, p.outline);
  fillRect(f, 6, 3, 6, 3, p.shirtDark);
  fillRect(f, 5, 12, 8, 1, p.belt);
  fillRect(f, 3, 2, 2, 5, p.skin);
  setPixel(f, 3, 2, p.outline);
  fillRect(f, 13, 4, 2, 7, p.skin);
  setPixel(f, 14, 4, p.outline);
  box(f, 5, 13, 8, 4, p.pants, p.outline);
...
function drawBigRun2(f: PixelImage, p: BodyPalette): void {
  box(f, 5, 3, 8, 10, p.shirt, p.outline);
  fillRect(f, 6, 4, 6, 3, p.shirtDark);
  fillRect(f, 5, 13, 8, 1, p.belt);
  fillRect(f, 3, 3, 2, 7, p.skin);
  setPixel(f, 3, 3, p.outline);
  fillRect(f, 13, 3, 2, 7, p.skin);
  setPixel(f, 14, 3, p.outline);
  box(f, 5, 14, 8, 4, p.pants, p.outline);
...
function drawBigSkid(f: PixelImage, p: BodyPalette): void {
  box(f, 3, 2, 8, 10, p.shirt, p.outline);
  fillRect(f, 4, 3, 6, 3, p.shirtDark);
  fillRect(f, 3, 12, 8, 1, p.belt);
  fillRect(f, 1, 3, 2, 7, p.skin);
  setPixel(f, 1, 3, p.outline);
  fillRect(f, 11, 2, 3, 5, p.skin);
  setPixel(f, 13, 2, p.outline);
  box(f, 3, 13, 8, 5, p.pants, p.outline);
...
function drawBigJump(f: PixelImage, p: BodyPalette): void {
  box(f, 4, 1, 8, 10, p.shirt, p.outline);
  fillRect(f, 5, 2, 6, 3, p.shirtDark);
  fillRect(f, 4, 11, 8, 1, p.belt);
  fillRect(f, 2, 0, 2, 6, p.skin);
  setPixel(f, 2, 0, p.outline);
  fillRect(f, 12, 0, 2, 6, p.skin);
  setPixel(f, 13, 0, p.outline);
  box(f, 4, 12, 8, 5, p.pants, p.outline);
...
function drawBigLand(f: PixelImage, p: BodyPalette): void {
  box(f, 4, 8, 8, 8, p.shirt, p.outline);
  fillRect(f, 5, 9, 6, 3, p.shirtDark);
  ...
  box(f, 4, 17, 8, 4, p.pants, p.outline);
...
function drawBigHurt(f: PixelImage, p: BodyPalette): void {
  box(f, 3, 3, 8, 10, p.shirt, p.outline);
  fillRect(f, 4, 4, 6, 3, p.shirtDark);
  fillRect(f, 3, 13, 8, 1, p.belt);
  fillRect(f, 0, 2, 3, 5, p.skin);
  setPixel(f, 0, 2, p.outline);
  fillRect(f, 11, 1, 3, 5, p.skin);
  setPixel(f, 13, 1, p.outline);
  box(f, 4, 14, 7, 5, p.pants, p.outline);
...
function drawBigDead(f: PixelImage, p: BodyPalette): void {
  box(f, 3, 8, 10, 8, p.shirt, p.outline);
  fillRect(f, 4, 9, 8, 3, p.shirtDark);
  fillRect(f, 3, 16, 10, 1, p.belt);
  fillRect(f, 1, 9, 2, 7, p.skin);
  setPixel(f, 1, 9, p.outline);
  fillRect(f, 13, 9, 2, 7, p.skin);
  setPixel(f, 14, 9, p.outline);
  box(f, 4, 17, 8, 5, p.pants, p.outline);
...
function main(): void {
  ...
  const writeSet = (suffix: string, palette: BodyPalette) => {
    // We need to re-bind the drawers with the palette
    const smallSheet = buildSheet(SMALL_DRAWERS.map(d => (f) => d(f, palette)), FRAME_W, FRAME_H_SMALL);
    writePng(path.join(spritesDir, `bart_body_small${suffix}.png`), smallSheet);
    console.log(`wrote bart_body_small${suffix}.png (${smallSheet.width}x${smallSheet.height})`);

    const bigSheet = buildSheet(BIG_DRAWERS.map(d => (f) => d(f, palette)), FRAME_W, FRAME_H_BIG);
    writePng(path.join(spritesDir, `bart_body_big${suffix}.png`), bigSheet);
    console.log(`wrote bart_body_big${suffix}.png (${bigSheet.width}x${bigSheet.height})`);
  };

  writeSet('', PALETTE_REGULAR);
  writeSet('_fire', PALETTE_FIRE);
